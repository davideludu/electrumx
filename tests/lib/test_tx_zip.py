...
def test_zip_v2_tx():
    test = bfh(V2_TX)
    deser = lib_tx_zip.DeserializerZip(test)
    tx = deser.read_tx()
    assert tx.version == 2
    assert tx.tx_type == 0
...
def test_zip_tx_pro_up_serv_tx():
...
def test_zip_tx_pro_up_serv_tx_v2():
...
def test_zip_tx_unknown_spec_tx():
...
def test_zip_tx_serialize_wrong_tx_type():
    test = bfh(CB_TX)
    deser = lib_tx_zip.DeserializerZip(test)
    tx = deser.read_tx()
    assert tx.tx_type == 5
    ...
    with pytest.raises(ValueError) as excinfo:
        ser = tx.serialize()
    assert ('Zip tx_type does not conform'
